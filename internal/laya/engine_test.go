...
func TestManagedEngineUsesBuiltinVerifiedClassifier(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(source, "model.json"), data, 0600); err != nil {
		t.Fatal(err)
	}
	...
	manifest := models.Manifest{ID: models.DefaultModelID, Language: models.DefaultLanguage, Version: "v1", Runtime: "builtin-go", Artifacts: []models.Artifact{{Path: "model.json", SHA256: hex.EncodeToString(hash[:])}}}
	if err := manager.Install(manifest, source, true); err != nil {
		t.Fatal(err)
	}
...
func TestManagedEngineFallsBackInProcessWhenModelUnavailable(t *testing.T) {
	result, err := (ManagedEngine{Fallback: FallbackEngine{}}).Evaluate(Request{Language: ProtocolLanguage, State: map[string]any{"task": "inspect"}, Questions: []Question{{ID: "backend", Kind: Choice, Options: []string{"agy", "qwen"}, Fallback: "qwen"}}})
	if err != nil {
		t.Fatal(err)
	}
	if !result.Fallback || result.Decision == nil || result.Error == "" {
